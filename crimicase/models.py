...
from baseinfo.constants import PROPERTY_CHOICES, INFO_TYPE, CASE_PROPERTY, SEX_CHOICES, JUDGE_RESULT, IS_ASSIGN
from baseinfo.models import CriminalStage
from civilcase.utils import money_num2word
from crimicase.constants import CRI_EVI_TYPE, CRI_EVI_SOURCE, CRI_ASK_TARGET, ENFORCE_WAY, ORG_CRIMINAL_LEGAL_STATUS
...
class Criminal(models.Model):
    ...
    enforce_way = models.IntegerField(verbose_name='强制措施', choices=ENFORCE_WAY, blank=True, null=True)
    detain_time = models.DateField(verbose_name='拘留时间', blank=True, null=True)
    arrest_time = models.DateField(verbose_name='批捕时间', blank=True, null=True)
    detention_place = models.CharField(verbose_name='羁押地点', max_length=200, blank=True, null=True)
    org_legal_status = models.IntegerField(verbose_name="原审法律地位", choices=ORG_CRIMINAL_LEGAL_STATUS, blank=True, null=True)
    stage = models.ForeignKey(CriminalStage, related_name="stages", verbose_name='案件所处阶段')
    commission_stage = models.ManyToManyField(CriminalStage, verbose_name='委托阶段', blank=True)
    ...
    party_person_status = models.IntegerField(verbose_name="所处法律地位", choices=ORG_CRIMINAL_LEGAL_STATUS, blank=True, null=True)
    ...
    party_company_status = models.IntegerField(verbose_name="所处法律地位", choices=ORG_CRIMINAL_LEGAL_STATUS, blank=True, null=True)
    ...
    # 分配律师
    lawyer = models.ForeignKey(UserInfo, verbose_name='主办律师', related_name="crimi_lawyers", blank=True, null=True, on_delete=models.SET_NULL)
    assist = models.ForeignKey(UserInfo, verbose_name='辅助律师', related_name="crimi_assists", blank=True, null=True,
                               on_delete=models.SET_NULL)
    manager = models.ForeignKey(UserInfo, verbose_name='客户经理', related_name="crimi_managers", blank=True, null=True,
                                on_delete=models.SET_NULL)
...
class CriminalHisResult(models.Model):
    """刑事历史判决结果"""
    MANNER = (
        (0, '接受'),
        (1, '不服'),
    )
    criminal = models.ForeignKey(Criminal, verbose_name='案件名称', on_delete=models.CASCADE)
    stage = models.ForeignKey(CriminalStage, verbose_name='案件阶段', )
    ...
    def __str__(self):
        return "{}({})".format(self.criminal.case_name, self.stage.name)
...
class CriminalDetail(models.Model):
    ...
    criminal = models.ForeignKey(Criminal, verbose_name='案件名称', on_delete=models.CASCADE)
    legal_status = models.IntegerField(verbose_name="法律地位", choices=ORG_CRIMINAL_LEGAL_STATUS)
    stage = models.ForeignKey(CriminalStage, verbose_name='执行阶段', )
    # 公安
    police_name = models.CharField(verbose_name='公安名称', max_length=50, blank=True, null=True)
    police_tel = models.CharField(verbose_name='公安电话', max_length=30, blank=True, null=True)
    police_person = models.CharField(verbose_name='公安联系人', max_length=20, blank=True, null=True)
    # 检察院
    procuracy_name = models.CharField(verbose_name='检察院名称', max_length=50, blank=True, null=True)
    procuracy_tel = models.CharField(verbose_name='检察院电话', max_length=30, blank=True, null=True)
    procuracy_person = models.CharField(verbose_name='检院联系人', max_length=20, blank=True, null=True)
    procuracy_result = models.CharField(verbose_name='审查结果', max_length=600, blank=True, null=True)
    # 法院
    court_name = models.CharField(verbose_name='法院名称', max_length=50, blank=True, null=True)
...
class CrimiNaturePerson(models.Model):
    """刑事自然人信息"""
    case = models.ForeignKey(CriminalDetail, verbose_name='案件名称', related_name="natures", null=True, blank=True, on_delete=models.CASCADE,)
...
class CrimiLegalPerson(models.Model):
    """刑事法人信息"""
    case = models.ForeignKey(CriminalDetail, verbose_name='案件名称',  related_name="legals", on_delete=models.CASCADE,)
...
class CrimiLawsuitScheme(models.Model):
    """刑事诉讼方案"""
    crimi_detail = models.OneToOneField(CriminalDetail, verbose_name='案件名称', on_delete=models.CASCADE)
    legal_opinion = models.TextField(verbose_name='法律意见', blank=True, null=True, help_text="法律意见书")
    bail_reasons = models.TextField(verbose_name='申请理由', blank=True, null=True, help_text="取保候审申请书")
    no_arrest_opinion = models.TextField(verbose_name='法律意见', blank=True, null=True, help_text="不予逮捕法律意见书")
    detain_reason = models.TextField(verbose_name='羁押申请理由', blank=True, null=True, help_text="羁押必要性审查申请")
    ...
    retrialapply_claim = models.TextField(verbose_name='申请事项', blank=True, null=True, help_text="再审申请书")
    ...
    def __str__(self):
        return self.crimi_detail


class CrimiEvidence(models.Model):
    """证据目录"""
    ...
    evi_type = models.IntegerField(verbose_name='证据类型', choices=CRI_EVI_TYPE, default=1)
    evi_source = models.IntegerField(verbose_name='证据来源', choices=CRI_EVI_SOURCE, default=1)
...
class CrimiAskingOutline(models.Model):
    """刑事询问提纲"""
    scheme = models.ForeignKey(CrimiLawsuitScheme, verbose_name='案件名称', on_delete=models.CASCADE)
...
class CrimiApplications(models.Model):
    """所需申请书"""
    scheme = models.ForeignKey(CrimiLawsuitScheme, verbose_name='案件名称', on_delete=models.CASCADE)
    appli_name = models.CharField(verbose_name='所需申请书', max_length=200)
    update_at = models.DateTimeField(verbose_name="修改时间", auto_now_add=True)
    create_at = models.DateTimeField(verbose_name="创建时间", auto_now=True)

    class Meta:
        verbose_name = '刑事所需申请书'
        verbose_name_plural = verbose_name

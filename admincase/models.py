...
class AdminCase(models.Model):
    ...
    case_stage = models.ForeignKey(CivilLevel, related_name="admin_stages", verbose_name='案件所处阶段')
    ...
    party_person_status = models.ForeignKey(LegalStatus, verbose_name="所处法律地位", related_name="admin_person_status", blank=True, null=True, on_delete=models.SET_NULL)
    ...
    party_company_status = models.ForeignKey(LegalStatus, verbose_name="所处法律地位", related_name="admin_company_status", blank=True, null=True,
    ...
    # 分配律师
    lawyer = models.ForeignKey(UserInfo, verbose_name='主办律师', related_name="admin_lawyers", blank=True, null=True, on_delete=models.SET_NULL)
    assist = models.ForeignKey(UserInfo, verbose_name='辅助律师', related_name="admin_assists", blank=True, null=True,
                               on_delete=models.SET_NULL)
    manager = models.ForeignKey(UserInfo, verbose_name='客户经理', related_name="admin_managers", blank=True, null=True,
                                on_delete=models.SET_NULL)
    ...
    @classmethod
    def get_max_sn(cls):
        year = datetime.datetime.now().strftime('XZ%Y')
        maxVal = cls.objects.filter(case_id__startswith=year).aggregate(sn_max=Max("case_id"))
        max_value = maxVal['sn_max']
        if maxVal['sn_max'] is None:
            num = '0001'
            case_id = '{0}{1}'.format(year, num)
            return case_id
        else:
...
class AdminHisResult(models.Model):
    """行政历史判决结果"""
    MANNER = (
        (0, '接受'),
        (1, '不服'),
    )
    admin = models.ForeignKey(AdminCase, verbose_name='案件名称', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return "{}({})".format(self.admin.case_name, self.stage.name)
...
class AdminNaturePerson(models.Model):
    """行政自然人信息"""
    case = models.ForeignKey(AdminDetail, verbose_name='案件名称', related_name="natures", null=True, blank=True, on_delete=models.CASCADE,)
...
class AdminLegalPerson(models.Model):
    """行政法人信息"""
    case = models.ForeignKey(AdminDetail, verbose_name='案件名称',  related_name="legals", on_delete=models.CASCADE,)
...
class AdminLawsuitScheme(models.Model):
    """行政诉讼方案"""
    admin_detail = models.OneToOneField(AdminDetail, verbose_name='案件名称', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return self.admin_detail


class AdminCompensate(models.Model):
    """赔偿明细"""
    scheme = models.ForeignKey(AdminLawsuitScheme, verbose_name='案件诉讼方案', on_delete=models.CASCADE)
...
class AdminEvidence(models.Model):
    """证据目录"""
    scheme = models.ForeignKey(AdminLawsuitScheme, verbose_name='案件诉讼方案', on_delete=models.CASCADE)
...
class AdminAskingOutline(models.Model):
    """行政询问提纲"""
    scheme = models.ForeignKey(AdminLawsuitScheme, verbose_name='案件诉讼方案', on_delete=models.CASCADE)
